...
require 'fpdf/fpdf.php';
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Ln(3);
			$this->Cell(90);
			$this->Cell(90,10, 'Reporte Proveedores',1,0,'C');
	...
	$pdf->Cell(40,6,'E-Mail',1,1,'C',1);

	$pdf->SetFont('Arial','',10);

	$R=mysqli_query($db,"SELECT * FROM `proveedores`");
	while($row=mysqli_fetch_array($R))
	{
		$pdf->Cell(10,6,utf8_decode($row['idProveedor']),1,0,'C');
		...
		$pdf->Cell(20,6,utf8_decode($row['cp']),1,0,'C');
		$pdf->Cell(40,6,utf8_decode($row['mail']),1,1,'C');
	}
	$pdf->Output();}else{
